...
describe Projet do
  ...
  it "Chaque résumé doit être complet" do
    FactoryGirl.build(:projet,etat: :lance, resumes: [@resume1,@resume2], derive_cout: 0, derive_delai: 0).should be_valid
    @resume2.cout = nil
    FactoryGirl.build(:projet,etat: :lance, resumes: [@resume1,@resume2], derive_cout: 0, derive_delai: 0).should be_invalid
  end
  it "S'il y a plusieurs études, la dérive des coûts est servie" do
    FactoryGirl.build(:projet,etat: :lance, resumes: [@resume1,@resume2], derive_cout: nil, derive_delai: 10).should be_invalid
  end
