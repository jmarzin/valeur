...
class CoherenceProjet < ActiveModel::Validator
  def validate(rec)
    if rec.class == Projet then
      if [:en_cours,:termine,:arrete].include?(rec.etat) && rec.resumes.empty? then
      ...
      if nb <= 1 || (nb == 2 && rec.resumes[0].stade == :avant_projet) 
        if rec.derive_cout then rec.errors[:base] << "Le calcul de la dérive des coûts nécessite 2 vraies études" end
        if rec.derive_duree then rec.errors[:base] << "Le calcul de la dérive de la durée nécessite 2 vraies études" end
      else 
        if not rec.derive_cout then rec.errors[:base] << "La dérive des coûts est calculée s'il y a plusieurs vraies études" end
        if not rec.derive_duree then rec.errors[:base] << "La dérive de la durée est calculée s'il y a plusieurs vraies études" end
      end
      ...
      if not rec.publie then rec.date_publication = nil end
      rec.errors[:base] << rec.stade.to_s+" invalide" unless rec.stade.to_s =~ /^(suivi\d\d|avant_projet|projet|bilan)$/
      if rec.type_produit == :specifique && (rec.duree_vie == nil || rec.duree_vie == 0)
        ...
        rec.duree_vie_totale = rec.duree_vie + rec.duree_projet if rec.duree_projet
      end
